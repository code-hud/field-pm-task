...
Open **Jaeger at http://localhost:16686**, pick the `trading-api` service and **Find
Traces**. Click a trace for the waterfall — the HTTP request, its call to `fraud-service`,
and the `pg` query spans underneath.
...
- Traces go to the bundled Jaeger. The exporter wiring lives in each service's `otel.js`.
